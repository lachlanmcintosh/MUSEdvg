...
EPSILON = 0.001
UPPER_BOUND = 4
TIME_LIMIT = 2 # this is the time limit in second to look for a solution in each round.
MAX_ITER = 2
PLOIDYS = [x/2.0 for x in range(1,10)]
...
for PLOIDY in PLOIDYS:
  print "inital ploidy estimate: "+str(PLOIDY)
  Wit = normal
  if num_normals > 1:
  ...
  while i < MAX_ITER:
    ...
    print("SCORE")
    print(np.sum((np.dot(Wit,Xit) - Z)**2))

    if PLOIDY == PLOIDYS[0] or np.sum((np.dot(Wit,Xit) - Z)**2) < np.sum((np.dot(Wit_best,Xit_best) - Z)**2):
      PLOIDY_best = PLOIDY
      Wit_best = Wit
      Xit_best = Xit
